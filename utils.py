...
def produce_require_dict(js_dependencies, js_host) -> dict:
    ...
    for name in js_dependencies.items:
        if name.startswith("https://prod-xine-plus.lingreninvestment.com"):
            confs.append("'prod_map_api{}':'{}'".format(len(name), name))
            libraries.append("'prod_map_api{}'".format(len(name)))
